...
import {
    ...
    getSingleton,
} from '../../representation/sync/syncLinkedRepresentation';
    ...
    describe('named collection on a parent resource', () => {
        ...
        describe('item in collection (getResourceInNamedCollection)', () => {
            it('should not update when the document (as an item) is the same as that in the list', async () => {
                ...
                const result = await getResourceInNamedCollection(
        ...
        describe('getCollectionInNamedCollection', () => {
            it('should not update when the collections (all items) are the same', async () => {
                ...
                const result = await getCollectionInNamedCollection(makeHydratedResource(parent), 'todos', /todos/, noChangeCollection, [], options);
            ...
            it('should update when the document matches an item and an attribute is different', async () => {
                ...
                const result = await getCollectionInNamedCollection(
                    makeHydratedResource(parent),
                    'todos',
                    /todos/,
                    oneItemChangedInCollection,
                    [],
                    options);
            ...
            it('should delete when a document is not found in collection', async () => {
                ...
                const result = await getCollectionInNamedCollection(makeHydratedResource(parent), 'todos', /todos/, oneItemRemovedInCollection, [], options);
        ...
        describe('getNamedCollectionInNamedCollection', () => {
            ...
            it('should not update when the collections (all items) are the same', async () => {
                ...
                const result = await getNamedCollectionInNamedCollection(
                    makeHydratedResource(parent),
                    'todos',
                    /todos/,
                    noChangeParentCollection,
                    [],
                    options);
        ...
        describe('getSingleton', () => {
            ...
            it('should not update when attributes on singleton are same', async function() {
                ...
                const result = await getSingleton(hydratedParent, 'user', /user/, noChangeParent, [], options);
                expect(result).toBeDefined();
                verifyMocks(2, 0, 0, 0);
            ...
            it('should update when attributes on singleton are different', async function() {
                ...
                const result = await getSingleton(hydratedParent, 'user', /user/, changedSingletonOnParent, [], options);
                expect(result).toBeDefined();
                verifyMocks(2, 0, 1, 0);
                assertThat(result).is(hydratedParent);
            });
